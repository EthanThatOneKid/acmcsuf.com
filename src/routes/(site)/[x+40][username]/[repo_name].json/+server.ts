...
export async function GET({ params, url }: RequestEvent<RouteParams>) {
  ...
  return handleGetRepositoryCertificatePageData(
    params.repo_name,
    params.username,
  );
}

async function handleGetRepositoryCertificatePageData(
  repoName: string,
  username: string,
) {
  ...
  data = await getRepositoryCertificatePageData(
    makeRepositoryCertificateQuery(repoName, username),
  );
  cache ??= new Map();
  cache.set(key, data);
...
async function handleGetReleaseCertificatePageData(
  username: string,
  release: string,
) {
  ...
  data = await getReleaseCertificatePageData(
    makeReleaseCertificateQuery(username, release),
  );
  cache ??= new Map();
  cache.set(key, data);
  cachedPageData.set(cache);
