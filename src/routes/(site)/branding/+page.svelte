<script lang="ts">
  import Spacing from '$lib/public/legacy/spacing.svelte';
  import { rgbToHex } from '@mui/material';

  import TeamCard from './team-card.svelte'

  const cards = [
    {
      title: "General",
      description: "The ACM general team is a dynamic group of individuals driving the success of our organization." +
      " ACM General manages operations, organizes events, and ensure the smooth functioning of ACM." +
      "They are the backbone of our community, fostering collaboration and innovation among members.", 
      teamLogos: [
        {
          image: '/assets/acm-csuf-badge.png',
          alt: 'General Team Bade 1',
          title: 'General Badge',
          svg: '/path-to-download/general-logo1.svg',
          png: '/path-to-download/general-logo1.png',
        },
        {
          image: '/assets/general-logo.svg',
          alt: 'General Team Logo 2 ',
          title: 'General Logo ',
          svg: '/assets/general-logo.svg',
          png: '/path-to-download/general-logo2.png',
        },
        {
          image: '/path-to-image/general-logo3.svg',
          alt: 'General Team Logo 3',
          title: 'General Logo White',
          svg: '/path-to-download/general-logo3.svg',
          png: '/path-to-download/general-logo3.png',
        },
        {
          image: '/path-to-image/general-logo4.svg',
          alt: 'General Team Logo 4',
          title: 'General Logo Heart',
          svg: '/path-to-download/general-logo4.svg',
          png: '/path-to-download/general-logo4.png',
        },
      ],
      teamColor: {
        name: 'Primary Blue',
        hex: '#06a3ff',
        rgb: '6, 163, 255', 
      },
    },
    {
      title: "AI",
      description: "The artificial intelligence team is dedicated to providing accessible information" +
      "about artificial intelligence and machine learning to all. AI focuses on fun projects " +
      "geared towards beginners in the field.", 
      teamLogos: [
        {
          image: '/path-to-image/ai-logo1.svg',
          alt: 'AI Team Logo 1',
          title: 'AI Badge',
          svg: '/path-to-download/ai-logo1.svg',
          png: '/path-to-download/ai-logo1.png',
        },
        {
          image: '/assets/ai-logo.svg',
          alt: 'AI Team Logo 2',
          title: 'AI Logo',
          svg: '/path-to-download/ai-logo2.svg',
          png: '/path-to-download/ai-logo2.png',
        },
        {
          image: '/path-to-image/ai-logo3.svg',
          alt: 'AI Team Logo 3',
          title: 'AI Logo White',
          svg: '/path-to-download/ai-logo3.svg',
          png: '/path-to-download/ai-logo3.png',
        },
        {
          image: '/path-to-image/ai-logo4.svg',
          alt: 'AI Team Logo 4',
          title: 'AI Logo Heart',
          svg: '/path-to-download/ai-logo4.svg',
          png: '/path-to-download/ai-logo4.png',
        },
      ],
      teamColor: {
        name: 'AI Green',
        hex: '#35e792',
        rgb: '53, 231, 146',
      },
    },
    {
      title: "Algo",
      description: "The algorithm team is dedicated to building programming fundamentals within students. " +
      "Algo focuses on mastering data structures and algorithms, enhancing problem solving abilities, and" +
      " exploration of competitive programming.",
      teamLogos: [
        {
          image: '/path-to-image/algo-logo1.svg',
          alt: 'Algo Team Badge',
          title: 'Algo Logo Badge',
          svg: '/path-to-download/algo-logo1.svg',
          png: '/path-to-download/algo-logo1.png',
        },
        {
          image: '/assets/algo-logo.svg',
          alt: 'Algo Team Logo',
          title: 'Algo Logo 2',
          svg: '/path-to-download/algo-logo2.svg',
          png: '/path-to-download/algo-logo2.png',
        },
        {
          image: '/path-to-image/algo-logo3.svg',
          alt: 'Algo Team Logo White',
          title: 'Algo Logo 3',
          svg: '/path-to-download/algo-logo3.svg',
          png: '/path-to-download/algo-logo3.png',
        },
        {
          image: '/path-to-image/algo-logo4.svg',
          alt: 'Algo Team Logo Heart',
          title: 'Algo Logo 4',
          svg: '/path-to-download/algo-logo4.svg',
          png: '/path-to-download/algo-logo4.png',
        },
      ],
      teamColor: {
        name: 'Algo Purple',
        hex: '#af3fff',
        rgb: '175, 63, 255',
      },
    },
    {
      title: "Open Source",
      description: "Our inspiration for this team surrounds this area of exploring space." +
      "The world of open source programming is endless and diving into it might seem " +
      "scary at first, but we find that discovering new things to be exciting. To best " +
      "represent this, our logo was designed as a rocket ship to represent the ability to launch" +
      "into the vast expanse of collaborative development, exploring new frontiers and " +
      "innovating together", 
      teamLogos: [
        {
          image: '/path-to-image/open-source-badge.svg',
          alt: 'Open Source Badge',
          title: 'Open Source Badge',
          svg: '/path-to-download/open-source-badge.svg',
          png: '/path-to-download/open-source-badge.png',
        },
        {
          image: '/assets/oss-logo.svg',
          alt: 'Open Source Logo',
          title: 'Open Source Logo',
          svg: '/path-to-download/open-source-logo.svg',
          png: '/path-to-download/open-source-logo.png',
        },
        {
          image: '/path-to-image/open-source-white-logo.svg',
          alt: 'Open Source White Logo',
          title: 'Open Source White Logo',
          svg: '/path-to-download/open-source-white-logo.svg',
          png: '/path-to-download/open-source-white-logo.png',
        },
        {
          image: '/path-to-image/open-source-heart.svg',
          alt: 'Open Source Heart',
          title: 'Open Source Heart',
          svg: '/path-to-download/open-source-heart.svg',
          png: '/path-to-download/open-source-heart.png',
        },
      ],
      teamColor: {
        name: 'OSS Green',
        hex: '#13d4b1',
        rgb: '19, 212, 177)',
      },
    },
    {
      title: "Design",
      description: "The design team is dedicated to emphasizing the importance of product design " +
      "and product management in the tech industry. Design focuses on educating students about " +
      "design principles, design tools, and the intricacies of conceptualization, development, "+
      "and management of a product.", 
      teamLogos: [
        {
          image: '/path-to-image/general-logo1.svg',
          alt: 'Design Team',
          title: 'Design Badge',
          svg: '/path-to-download/general-logo1.svg',
          png: '/path-to-download/general-logo1.png',
        },
        {
          image: '/assets/design-logo.svg',
          alt: 'AI Team Logo 2',
          title: 'Design Logo',
          svg: '/path-to-download/general-logo2.svg',
          png: '/path-to-download/general-logo2.png',
        },
        {
          image: '/path-to-image/general-logo3.svg',
          alt: 'AI Team Logo 3',
          title: 'Design Logo White',
          svg: '/path-to-download/general-logo3.svg',
          png: '/path-to-download/general-logo3.png',
        },
        {
          image: '/path-to-image/general-logo4.svg',
          alt: 'AI Team Logo 4',
          title: 'Design Heart',
          svg: '/path-to-download/general-logo4.svg',
          png: '/path-to-download/general-logo4.png',
        },
      ],
      teamColor: {
        name: 'Design Red',
        hex: '#ff406d',
        rgb:'255, 64, 109', 
      },
    },
    {
      title: "Dev",
      description: "The development team is dedicated to giving students the opportunity to explore tech " +
      "via hands-on projects and activities. Dev focuses on introducing students to software development, and" + 
      " the various tech stacks used in the industry.", 
      teamLogos: [
        {
          image: '/path-to-image/general-logo1.svg',
          alt: 'Dev Team',
          title: 'Dev Badge',
          svg: '/path-to-download/general-logo1.svg',
          png: '/path-to-download/general-logo1.png',
        },
        {
          image: '/assets/dev-logo.svg',
          alt: 'Dev Team Logo 2',
          title: 'Dev Logo',
          svg: '/path-to-download/general-logo2.svg',
          png: '/path-to-download/general-logo2.png',
        },
        {
          image: '/path-to-image/general-logo3.svg',
          alt: 'Dev Team Logo 3',
          title: 'Dev Logo White',
          svg: '/path-to-download/general-logo3.svg',
          png: '/path-to-download/general-logo3.png',
        },
        {
          image: '/path-to-image/general-logo4.svg',
          alt: 'Dev Team Logo 4',
          title: 'Dev Heart',
          svg: '/path-to-download/general-logo4.svg',
          png: '/path-to-download/general-logo4.png',
        },
      ],
      teamColor: {
        name: 'Dev Blue',
        hex: '#1F6CFF',
        rgb: '31, 108, 255',
      },
    },
    { 
      title: "Game Dev",
      description: "The game development team is dedicated to teaching the basics of programming in the " +
      "gaming industry. Gamedev focuses on educating students about design principles, design tools," +
      " and the development process of a project.", 
      teamLogos: [
        {
          image: '/path-to-image/general-logo1.svg',
          alt: 'Game Dev Team',
          title: 'Game Dev Badge',
          svg: '/path-to-download/general-logo1.svg',
          png: '/path-to-download/general-logo1.png',
        },
        {
          image: '/assets/gamedev-logo.svg',
          alt: 'Game Dev Team Logo 2',
          title: 'Game Dev Logo',
          svg: '/path-to-download/general-logo2.svg',
          png: '/path-to-download/general-logo2.png',
        },
        {
          image: '/path-to-image/general-logo3.svg',
          alt: 'Game Dev Team Logo 3',
          title: 'Game Dev Team Logo White',
          svg: '/path-to-download/general-logo3.svg',
          png: '/path-to-download/general-logo3.png',
        },
        {
          image: '/path-to-image/general-logo4.svg',
          alt: 'Game Dev Logo 4',
          title: 'Game Dev Heart',
          svg: '/path-to-download/general-logo4.svg',
          png: '/path-to-download/general-logo4.png',
        },
      ],
      teamColor: {
        name: 'Game Dev Red',
        hex: '#D41253', 
        rgb: '212, 18, 83', 
      },
    },
    { 
      title: "ICPC",
      description: "The Intercollegiate Competitive Programming Competition is a contest to challenge students on their algorithms" + 
      " and problem solving skills. The ICPC Team is dedicated to preparing students for the competition by hosting " +
      "weekly practice sessions and mock contests.", 
      teamLogos: [
        {
          image: '/path-to-image/general-logo1.svg',
          alt: 'ICPC Team',
          title: 'ICPC Badge ',
          svg: '/path-to-download/general-logo1.svg',
          png: '/path-to-download/general-logo1.png',
        },
        {
          image: '/assets/icpc-logo.svg',
          alt: 'AI Team Logo 2',
          title: 'ICPC Logo',
          svg: '/path-to-download/general-logo2.svg',
          png: '/path-to-download/general-logo2.png',
        },
        {
          image: '/path-to-image/general-logo3.svg',
          alt: 'AI Team Logo 3',
          title: 'ICPC Logo White',
          svg: '/path-to-download/general-logo3.svg',
          png: '/path-to-download/general-logo3.png',
        },
        {
          image: '/path-to-image/general-logo4.svg',
          alt: 'AI Team Logo 4',
          title: 'ICPC Heart',
          svg: '/path-to-download/general-logo4.svg',
          png: '/path-to-download/general-logo4.png',
        },
      ],
      teamColor: {
        name: 'ICPC Orange',
        hex: '#FF7918',
        rgb: '255, 121, 24', 
      },
    },
    {
      title: "Marketing",
      description: "The marketing team has a strong passion towards advertising and spreading word on " +
      " all our ACM events. Marketing utilizes their expertise in digital strategies and creative storytelling " +
      "to display a welcoming environment to all students.", 
      teamLogos: [
        {
          image: '/path-to-image/general-logo1.svg',
          alt: 'Marketing Team',
          title: 'Marketing Badge',
          svg: '/path-to-download/general-logo1.svg',
          png: '/path-to-download/general-logo1.png',
        },
        {
          image: '/assets/marketing-logo.svg',
          alt: 'Marketing Logo 2',
          title: 'Marketing Logo ',
          svg: '/path-to-download/general-logo2.svg',
          png: '/path-to-download/general-logo2.png',
        },
        {
          image: '/path-to-image/general-logo3.svg',
          alt: 'Marketing Logo 3',
          title: 'Marketing Logo White',
          svg: '/path-to-download/general-logo3.svg',
          png: '/path-to-download/general-logo3.png',
        },
        {
          image: '/path-to-image/general-logo4.svg',
          alt: 'Marketing Logo 4',
          title: 'Marketing Heart',
          svg: '/path-to-download/general-logo4.svg',
          png: '/path-to-download/general-logo4.png',
        },
      ],
      teamColor: {
        name: 'Marketing Pink',
        hex: '#FE0FC9',
        rgb: '254, 15, 201', 
      },
    }, 
    {
      title: "Node Buds",
      description: "Connect & grow with node buds! Our big-little program pairs you with an experienced club " +
      "officer for social events, workshops, and guidance as you navigate the tech world. Build friendships, " +
      "skills, and have a blast!", 
      teamLogos: [
        {
          image: '/path-to-image/general-logo1.svg',
          alt: 'Node Buds Team',
          title: 'Node Buds Badge',
          svg: '/path-to-download/general-logo1.svg',
          png: '/path-to-download/general-logo1.png',
        },
        {
          image: '/assets/nodebuds-logo-old.svg',
          alt: 'Nodebuds Team Logo 2',
          title: 'Node Buds Logo',
          svg: '/path-to-download/general-logo2.svg',
          png: '/path-to-download/general-logo2.png',
        },
        {
          image: '/path-to-image/general-logo3.svg',
          alt: 'Nodebuds Team Logo 3',
          title: 'Node Buds Logo White',
          svg: '/path-to-download/general-logo3.svg',
          png: '/path-to-download/general-logo3.png',
        },
        {
          image: '/path-to-image/general-logo4.svg',
          alt: 'Nodebuds Team Logo 4',
          title: 'Node Buds Heart',
          svg: '/path-to-download/general-logo4.svg',
          png: '/path-to-download/general-logo4.png',
        },
      ],
      teamColor: {
        name: 'Node Buds Red',
        hex: '#D41253',
        rgb: '212, 18, 83', 
      },
    }
  ]

</script>

<svelte:head>
  <title>Branding | ACM at CSUF</title>
</svelte:head>


<Spacing --min="175px" --med="200px" --max="200px" />

<main class="responsive-width">

    <!-- branding title section  -->
    <section id="top">
      <div class="header">
        <div class="header-content">
          <h1 class="size-xl">BRANDING</h1>
          <p class="p-info">Find more information about our branding alongside a collection of resources and assets.</p>
        </div>
        <img src="/assets/acm-csuf-badge.png" alt="Branding Badge">
      </div>
    
      {#each cards as card}
        <TeamCard
          title={card.title}
          description={card.description}
          teamLogos={card.teamLogos}
          teamColor={card.teamColor}
        />
        <br>
      {/each}
    </section>

  <p>
    <a href="#top">Back to top</a> | <a href="/global.css">global.css</a>
  </p>
</main>


<style>
  .header {
    display: flex;
    flex-direction: row;
    align-items: flex-start; /* Align items to the top */
    justify-content: space-between; /* Ensure the image stays on the right */
    gap: 20px; /* Optional: Space between title section and image */
    width: 100%;
  }

  .header-content {
    display: flex;
    flex-direction: column;
    max-width: 60%; /* Adjust as necessary */
  }

  .header h1 {
    margin: 0;
    text-align: left;
  }

  .p-info {
    margin-top: 10px;
    text-align: left;
  }

  .header img {
    max-width: 30%;
    height: auto;
    align-self: flex-start;
  }

  .responsive-width {
    width: min(80ch, 80vw);
    margin: 0 auto;
    padding-left: 1em; /* Aligns the left padding */
  }

  main p {
    margin: 1em 1em 1em 0; /* Match left margin of p */
  }


  .responsive-width {
    width: min(80ch, 80vw);
    margin: 0 auto;
  }

  @media (max-width: 480px) {
    .responsive-width {
      width: 100%;
    }
  }

  p {
    margin: 1em 0;
    word-break: break-word;
  }

  main {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
  }

  main section {
    margin: 0 auto;
    margin-bottom: 50px;
  }

  main h1 {
    text-align: center;
    margin: 0;
  }

  main a {
    transition: 0.25s ease-in-out;
  }

  main:hover {
    color: var(--acm-blue);
  }
</style>
